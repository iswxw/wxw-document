...
- wc 统计文件（行数，字节数，行数） `cat 文件 | wc -l 统计文件行数` ` wc 文件...` 可以是多个文件
- ​
...
- ` awk '{print NR}' log.txt  | tail -n 1`   NR 每行的记录号，结合tail 取最后一行
- ` cat log.txt | wc -l`  结合wc 统计文件行数
- ` awk 'END{print NR}' log.txt` 只打印最后一行

##### （2）打印文件的最后2行

- `  tail -n 2 log.txt`  打印文件后2行。一般查看日志也可以使用 `tail -fn xxx log `   来持续输出
